...
const addToCart = () => {
    router.post(route('cart.add'), {
        ...
        onSuccess: () => {
            ...
            // возможно, здесь можно обновить store или локальный флаг
        }
    });
};
...
<!--                <img :src="product.imageUrl || 'https://via.placeholder.com/180'" alt="image">-->
        ...
        <div class="prices-buttons flex justify-between items-center text-black">
            ...
            <div class="flex items-center gap-3">
                <div>
<!--                   TODO: Активная кнопка(цвет поменять)-->
...
<!--                <div class="p-3 rounded-[10px] bg-primary-purple duration-200 hover:bg-dark-purple cursor-pointer">-->
...
<!--                </div>-->
                ...
                <div v-if="isInCart" class="p-3 rounded-[10px] bg-dark-purple hover:bg-secondary-purple duration-200 cursor-pointer text-white ">
                   <Link :href="$route('basket')">В корзине </Link>
                </div>
                <div v-else @click="addToCart" class="p-3 rounded-[10px] bg-primary-purple text-white hover:bg-dark-purple cursor-pointer">
                    Добавить
                </div>
            </div>

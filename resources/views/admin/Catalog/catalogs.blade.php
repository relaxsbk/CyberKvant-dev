            ...
            <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                ...
                @foreach($catalogs as $catalog)
                    <tr class="border-b border-gray-950 dark:bg-[#2B2D2D] hover:bg-gray-50 dark:hover:bg-[#2B2D2D]/50 duration-300">
                        ...
                        <td class="px-6 py-4">{{ $catalog->published }}</td>
                        <td class="px-6 py-4">{{ $catalog->created_at->format('d.m.Y') }}</td>
                        <td class="flex items-center px-6 py-4 gap-3">
                            {{-- Edit Modal --}}
                            <div x-data="{ showEdit{{ $catalog->id }}: false }">
                                <button @click="showEdit{{ $catalog->id }} = true"
                                ...
                                <div x-show="showEdit{{ $catalog->id }}"
                                     x-transition
                                     class="fixed inset-0 flex items-center justify-center z-50 bg-black/50 backdrop-blur-sm"
                                     style="display: none;">
                                    <div @click.away="showEdit{{ $catalog->id }} = false"
                                         class="bg-white dark:bg-[#1e1e1e] p-6 rounded-lg shadow-xl w-full max-w-lg overflow-y-auto max-h-[90vh]">
                                        <h2 class="text-xl mb-4">Редактировать #{{ $catalog->id }}</h2>
                                        ...
                                        <form method="POST" action="{{ route('admin.catalogs.update', $catalog) }}" enctype="multipart/form-data">
                                            ...
                                            <div class="mb-4">
                                                <label class="block mb-1 font-medium">Название</label>
                                                <input name="title" type="text" value="{{ old('title', $catalog->title) }}"
                                                       class="w-full px-4 py-2 border border-none rounded-md dark:bg-[#464646]/50 dark:text-white duration-300 ease-in-out focus:ring-primary-purple">
                                            </div>
                                            ...
                                            <div class="mb-4">
                                                <label class="block mb-1 font-medium">Описание</label>
                                                <textarea name="description" rows="9"
                                                          class="w-full px-4 py-2 border border-none rounded-md dark:bg-[#464646]/50 dark:text-white duration-300 ease-in-out focus:ring-primary-purple">{{ old('description', $catalog->description) }}</textarea>
                                            </div>
                                            ...
                                            <div class="mb-4">
                                                ...
                                                <select name="published"
                                                        class="w-full px-4 py-2 border border-none rounded-md dark:bg-[#464646]/50 dark:text-white duration-300 ease-in-out focus:ring-primary-purple">
                                                    <option value="0" class="bg-[#464646]" @selected($catalog->published === 0)>Черновик</option>
                                                    <option value="1" class="bg-[#464646]" @selected($catalog->published === 1)>Опубликован</option>
                                                </select>
                                            </div>
                                            ...
                                            <div class="flex justify-end gap-2">
                                                ...
                                                <button type="button" @click="showEdit{{ $catalog->id }} = false"
                                                        class="px-4 py-2 rounded-md bg-gray-700 hover:bg-gray-500 duration-300 dark:text-white">
                                                    Отмена
                                                </button>
